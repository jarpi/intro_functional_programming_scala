 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of list with 1 element === 0") {
    assert(sum(List(0)) === 0)
  }

  test("sum of a few negative numbers") {
    assert(sum(List(-1,-5,3)) === -3)
  }

  test("sum of repeated numbers") {
    assert(sum(List(0,0,3,3)) === 6)
  }
  ...
  test("max of list 1 element") {
    assert(max(List(0)) === 0)
  }

  test("max of negative numbers") {
    assert(max(List(-3,-5)) === -3)
  }

  test("max of repeating number") {
    assert(max(List(-3,-3,-5,-5)) === -3)
  }
